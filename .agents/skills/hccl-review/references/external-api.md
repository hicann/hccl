...
`include/`（对外公共头：`hccl.h` 算子 API、`hccl_mc2.h` MC2 自定义算子框架）的新增/修改 PR 必须逐条核查本文全部条目。模块结构变更（新增类/文件/目录）按文末「模块变更」核查。规范权威来源为根 `AGENTS.md` 第 3 节与 `docs/zh/architecture/architecture-brief.md`。
...
- 对外结构体布局变更（增删成员、改类型、调顺序）评估对 AI 框架适配层与既有二进制的兼容影响
- 64 位默认 8 字节对齐：`uint32_t` 后紧跟指针/`uint64_t` 会产生 4 字节空洞，建议指针紧凑排列、尾部填充优于中间空洞

## 命名一致性（对照仓内既有惯例）

对照 `include/hccl.h` 既有声明逐项比较新增/修改接口的命名与类型风格。

- typedef struct/enum 不带 tag 名（`typedef struct { ... } Name;`，禁 `tagXxx`）
- 枚举值前缀 = 类型名大写蛇形
- 结构体带统一模块前缀，不散落无前缀
- 对外 C 函数返回错误码宏（如 `HcclResult`），非裸 `int32_t`
- 整型宽度收敛：对外接口用定宽类型（`uint32_t`/`uint64_t`），不用 `int`/`long`

## 兼容性与解耦

- `include/` 变更须向后兼容（不删不改既有接口语义，对上支撑 AI 框架适配层）
- 数据面原语不得引入控制面强耦合；控制面不得依赖具体数据面算子实现
- 指针参数只读须加 `const`
- 接口新增须评估对外必要性：内部用途不放 `include/`，避免过早开放带来兼容负担；`experimental/` 的接口不进入 `include/`

## 修改一致性与资料同步

- 头文件符号修改后 grep 全部引用点（含 AI 框架适配层、MC2 自定义算子使用者）同步更新，防编译错误
