...
<!-- md-trans-meta sourceCommit=unknown translatedAt=2026-08-05T02:56:54.870Z pushedAt=2026-08-11T03:27:22.806Z -->
...
- **For Ascend 950PR/Ascend 950DT**: The supported configurations are listed below. Setting an unsupported value will trigger error reporting.
  ...
  - **AIV**: Communication operators are expanded and executed on the Vector Core.
    ...
    - This config supports only Broadcast, Reduce, AllReduce, ReduceScatter, Scatter, AllGather, AlltoAll, and AlltoAllV operators.
  ...
  - **CCU_MS**: Communication operators are expanded on the CCU, using CcuBuffer for memory read/write. Ascend 950PR does not support this config.
    ...
    - This config supports only Broadcast, Reduce, AllReduce, ReduceScatter, AllGather, AllGatherV, and ReduceScatterV operators, and currently supports only single-device use cases.

      - For Broadcast, AllGather, and AllGatherV operators, the supported data types are int8, uint8, int16, uint16, int32, uint32, int64, uint64, float16, float32, float64, and bfp16.

      - For Reduce, AllReduce, ReduceScatter, and ReduceScatterV operators, the supported data types are int16, int32, float16, float32, and bfp16.
...
- **For Atlas A3 training products/Atlas A3 inference products**: The following configurations are supported. Setting an unsupported value leads to using the default value.
  ...
  - **AIV**: Communication operators are expanded and executed on the Vector Core.
    ...
    - This config does not support parallel communicators (because multiple communicators cannot be configured in `AIV` mode simultaneously); otherwise, unexpected behavior may occur. You can set the operator expansion mode of a specific communicator to `AIV` through `HcclCommConfig` during initialization of the communicator with specific configurations.
    ...
    - Under this config, collective communication supports core control. Set the number of Vector Cores based on the concurrency of computation operators and communication operators in actual use.
      ...
      - For AllReduce, deterministic ReduceScatter, AlltoAll, AlltoAllV, and AlltoAllVC operators, you are advised to allocate at least *max\(2, ceil\(ranksize/20\)\)* Vector Cores, and the number of cores must be an even number (if the calculation result is odd, round it up to the next even number).
        ...
        If the number of allocated Vector Cores cannot meet the requirements of algorithm orchestration, HCCL reports an error and prompts the minimum number of Vector Cores required.
...
- **For Atlas A2 training products/Atlas A2 inference products**: The supported configurations are listed below. Setting an unsupported value leads to using the default value.
  ...
  - **AI_CPU**: Communication operators are expanded on the AI CPU, and the device automatically selects the corresponding scheduler based on the hardware model.
    ...
    **Note**
    ...
    In graph (Ascend IR) or graph capture (aclgraph) use cases, when the communication algorithm uses AI CPU mode, the number of concurrent graphs on a single device must not exceed 6. Otherwise, communication blocking may occur due to AI CPU cores being fully occupied.
  ...
  - **AIV**: Communication operators are expanded and executed on the Vector Core.
    ...
    - Under this config, collective communication supports core control. Set the Vector Core count based on the concurrency of computation operators and communication operators in actual use.
    ...
    **Note**
    ...
    - When the algorithm orchestration expansion is set to `AIV`, if [HCCL_DETERMINISTIC](HCCL_DETERMINISTIC.md) is set to `true` or `strict` to enable deterministic computation, deterministic computation takes higher priority, and AIV expansion may not take effect in certain use cases.
