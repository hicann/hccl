...
针对PyTorch和MindSpore框架，HCCL的调用已集成到TorchNPU和MindSpore框架代码中，开发者指定使用HCCL作为分布式后端，直接使用框架原生通信API，即可实现分布式能力，详细使用方法可参见《[TorchNPU 产品文档](https://www.hiascend.com/document/detail/zh/Pytorch/latest/index/index.html)》和[MindSpore官网](https://www.mindspore.cn/)。
