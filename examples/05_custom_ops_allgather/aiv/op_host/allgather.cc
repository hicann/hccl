...
static HcclResult
InitAivBuffer(HcclComm comm, const char* aivTag, void*& algCtxPtr, void*& aivCommInfoPtr, HcclMemHandle& memHandle)
{
    HCCL_INFO("[%s] init aiv buffer.", __func__);
    uint64_t algCtxSize = sizeof(AlgResourceCtx);
    auto hcclRet = HcclEngineCtxGet(comm, aivTag, CommEngine::COMM_ENGINE_AIV, &algCtxPtr, &algCtxSize);

    if (hcclRet != HCCL_SUCCESS) {
        hcclRet = HcclEngineCtxCreate(comm, aivTag, CommEngine::COMM_ENGINE_AIV, algCtxSize, &algCtxPtr);
        if (hcclRet != HCCL_SUCCESS) {
            HCCL_ERROR("[%s] Failed to create AIV buffer. ret=%d", __func__, hcclRet);
            return hcclRet;
        }
        aivCommInfoPtr = static_cast<AlgResourceCtx*>(algCtxPtr)->aivCommInfoPtr;
        ...
        HcclMemHandle handle = static_cast<HcclMemHandle>(algCtxPtr);
        handle = memHandle;
    } else {
        aivCommInfoPtr = static_cast<AlgResourceCtx*>(algCtxPtr)->aivCommInfoPtr;
        memHandle = static_cast<HcclMemHandle>(algCtxPtr);
    }

    return HCCL_SUCCESS;
}

static bool IsUbrMemProtocol(uint32_t protocol) { return protocol == COMM_PROTOCOL_UB_MEM; }

static HcclResult
BuildChannelRequests(HcclComm comm, uint32_t rank, uint32_t rankSize, std::vector<HcclChannelDesc>& channelRequests)
{
    HCCL_INFO("[%s] build channel.", __func__);
...
static HcclResult AcquireChannelsAndBuffers(
    HcclComm comm, HcclMemHandle& memHandle, std::vector<HcclChannelDesc>& channelRequests, void** buffersIn,
    void** buffersOut)
{
...
static HcclResult SetupRemoteChannels(
    HcclComm comm, uint32_t rank, uint32_t rankSize, HcclMemHandle& memHandle, void** buffersIn, void** buffersOut)
{
...
{
    void* algCtxPtr = nullptr;
    void* aivCommInfoPtr = nullptr;
    HcclMemHandle memHandle;
    CHK_RET(InitAivBuffer(comm, param.tag, algCtxPtr, aivCommInfoPtr, memHandle));
    ...
    ACLCHECK(aclrtMemcpy(
        aivCommInfoPtr, MAX_RANK_SIZE * sizeof(void*), buffersIn, MAX_RANK_SIZE * sizeof(void*),
        ACL_MEMCPY_HOST_TO_DEVICE));
    ACLCHECK(aclrtMemcpy(
        static_cast<uint8_t*>(aivCommInfoPtr) + AIV_TAG_ADDR_OFFSET, MAX_RANK_SIZE * sizeof(void*), buffersOut,
