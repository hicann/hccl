...
static HcclResult InitAivBuffer(HcclComm comm, const char* aivTag, void*& aivCommInfoPtr, HcclMemHandle& memHandle)
{
    ...
    // 访问 g_memHandleCache 时加锁：
    std::lock_guard<std::mutex> lock(g_memHandleCacheMutex);
...
HcclResult HcclReduceScatterCustom(
    ...
    aclrtStream stream)
{
    HCCL_INFO("[HcclReduceScatterCustom] Entry. recvCount=%lu sendBuf=%p recvBuf=%p", recvCount, sendBuf, recvBuf);
    ...
    char commName[COMM_IDENTIFIER_MAX_LENGTH];
    ...
    param.yRankSize = 0;
    param.zRankSize = 0;
    param.len = recvCount * SIZE_TABLE[dataType];
    ...
    HCCL_INFO("[HcclReduceScatterCustom] Launching kernel... rank=%u rankSize=%u", rank, rankSize);
    CHK_RET(LaunchKernel(param, stream));
    HCCL_INFO("[HcclReduceScatterCustom] Launch success");
