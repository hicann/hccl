...
HcclResult GetThreadForCcu(HcclComm comm, const OpParam& param, AlgResourceCtxSerializable& resCtxHost)
{
    ...
    CHK_RET(HcclThreadAcquireWithStream(
        comm, param.engine, param.stream, resCtxHost.notifyNumOnMainThread,
        &thread)); // host模式下，将主流封装为thread，并创建主流上的notify
...
HcclResult GetChannelForCcu(HcclComm comm, const OpParam& param, std::vector<ChannelHandle>& kernelChannels)
{
    ...
    for (uint32_t remoteRank = 0; remoteRank < param.rankSize; remoteRank++) {
        ...
        CHK_RET(HcclRankGraphGetLinks(
            comm, netLayer, param.myRank, remoteRank, &linkList,
        ...
        if (!protocolExists) {
            HCCL_ERROR(
                "[GetChannelForCcu] Protocol %d not found between rank %u and rank %u", protocol, param.myRank,
                remoteRank);
    ...
    const std::vector<ChannelHandle>& kernelChannels, CcuKernelInfo& kernelInfo)
{
    ...
    if (kernelChannels.size() > CCU_MAX_RANK_SIZE) {
        HCCL_ERROR("[GetCcuKernel] rankSize %u exceeds CCU_MAX_RANK_SIZE %lu", param.rankSize, CCU_MAX_RANK_SIZE);
        return HCCL_E_PARA;
    }
    ...
    CHK_PRT_RET(
        insNum != 1, HCCL_ERROR("[GetCcuKernel] HcclCommQueryCcuIns fail! insNum is [%u]", insNum), HCCL_E_INTERNAL);
    ...
    CcuResult regRet = HcommCcuKernelRegister(
        ...
        kernelArgNum, &kernelHandle); // 注册kernel
...
{
    // std::cout << "CalGoSize: loopCount=" << config.loopCount << ", memSlice=" << config.memSlice << std::endl;
    ...
    // std::cout << "CalGoSize: size=" << size << ", loopSize=" << loopSize << ", maxSize=" << maxSize << ", m=" << m <<
    // ", n=" << n << ", p=" << p << std::endl;
    ...
    // std::cout << "CalGoSize: offset=" << offset << ", loop=" << loopIterNum<< ", parallel=" << loopExtendNum <<
    // ",tail=" << tailSize << std::endl;
