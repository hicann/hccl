...
static void InitGroupCopyResources(
    ReduceScatterMesh1DMem2MemContext& ctx, ccu::LocalAddr* loopSrc, ccu::LocalAddr* loopDst, ccu::Variable* loopLen)
{
...
static CcuResult
GroupCopy(ReduceScatterMesh1DMem2MemContext& ctx, ccu::LocalAddr dst, ccu::LocalAddr src, GroupOpSizeVars goSize)
{
    ccu::LocalAddr loopSrc[2], loopDst[2];
...
static CcuResult ExecuteReduceScatterTransfer(ReduceScatterMesh1DMem2MemContext& ctx)
{
        ...
        for (uint64_t rankIdx = 0; rankIdx < ctx.arg->rankSize; rankIdx++) {
            ...
            if (rankIdx != ctx.arg->rankId) {
                CCU_CHK_RET(ccu::WriteReduce(
                    ctx.arg->channels[channelId], dst[rankIdx], src, ctx.sliceSize, ctx.dataType, ctx.reduceOp,
                    ctx.event, mask)); // 本卡数据写入远端地址
                channelId++;
                totalMask |= mask;
            }
        }
    ...
    const uint16_t localMask = 1 << ctx.arg->rankId;
    CCU_CHK_RET(ccu::LocalReduce(
        localDst, src, ctx.sliceSize, ctx.dataType, ctx.reduceOp, ctx.event, localMask)); // 本卡数据写入远端地址
    CCU_CHK_RET(ccu::EventRecord(ctx.event, localMask));
    CCU_CHK_RET(ccu::EventWait(ctx.event, localMask)); // 等待本卡数据搬运完成
...
CcuResult CcuReduceScatterMesh1DMem2MemKernel(CcuKernelArg arg)
{
    ...
    ctx.arg = kernelArg;
    ctx.dataType = kernelArg->opParam.dataType;
    ctx.reduceOp = kernelArg->opParam.reduceOp;

    if (ctx.arg->channelCount == 0) {
        HCCL_ERROR("[CcuReduceScatterMesh1DMem2MemKernel] channels is empty!");
    ...
    // 4.执行算法
    CCU_CHK_RET(ExecuteReduceScatterTransfer(ctx));
