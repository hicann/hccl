...
constexpr float TEST_DATA_VALUE = 2026;
...
// 递归终止函数
void BuildLogString(std::ostringstream& oss) {}

template <typename T, typename... Args>
void BuildLogString(std::ostringstream& oss, const T& first, const Args&... rest)
{
    oss << first;
    BuildLogString(oss, rest...);
}
...
int PrepareData(
    int rank, int rankSize, uint64_t count, size_t sendBytes, size_t recvBytes, aclrtStream& stream, void*& sendBuf,
    void*& recvBuf)
{
    ...
    std::vector<float> hostSend(count * rankSize, (float)(rank + TEST_DATA_VALUE));
...
int VerifyResult(int rank, int rankSize, uint64_t count, size_t recvBytes, void* recvBuf)
{
    ...
    float sum = (TEST_DATA_VALUE + TEST_DATA_VALUE + rankSize - 1) * rankSize / 2;

    for (uint64_t i = 0; i < count; i++) {
        float val = hostRecv[i];
        if (std::abs(val - sum) > 1e-5) {
            Log(rank, "Error at offset ", i, ": expected ", sum, ", got ", val);
            return -1;
        }
    }
...
int Sample(void* arg)
{
    ...
    size_t sendBytes = dataLen * rankSize * sizeof(float);
    size_t recvBytes = dataLen * sizeof(float);
    size_t recvSize = dataLen;

    aclrtStream stream = nullptr;
    void *sendBuf = nullptr, *recvBuf = nullptr;

    HCCLCHECK(PrepareData(rank, rankSize, dataLen, sendBytes, recvBytes, stream, sendBuf, recvBuf));

    auto start = high_resolution_clock::now();
    // 执行 ReduceScatter，对通信域内所有 rank 的 sendBuf 按 rank 分片做规约（SUM），将各 rank
    // 对应分片的规约结果发送到对应 rank 的 recvBuf
    HCCLCHECK(
        HcclReduceScatterCustom(sendBuf, recvBuf, dataLen, HCCL_DATA_TYPE_FP32, HCCL_REDUCE_SUM, hcclComm, stream));
    // 阻塞等待任务流中的集合通信任务执行完成
    ...
    // 接收数据校验
    int ret = (VerifyResult(rank, rankSize, dataLen, recvBytes, recvBuf));
...

    {
        ...
        std::cout << "rankId: " << rank << ", output: [" << std::flush;
        ;
        for (uint32_t i = 0; i < std::min(64ul, recvSize); ++i) {
            std::cout << " " << tmpResBuff[i] << std::flush;
            ;
        }
