...
#include "ccu_kernel_all_reduce_experimental_mesh1d.h"
...
static CcuResult
ParseKernelArg(AllReduceExperimentalMesh1DContext& ctx, CcuKernelArgAllReduceExperimentalMesh1D* kernelArg)
{
    ...
    if (ctx.outputDataType == HcclDataType::HCCL_DATA_TYPE_RESERVED) {
        ...
        HCCL_DEBUG(
            "[CcuAllReduceExperimentalMesh1D] outputDataType is [INVALID], set outputDataType to[%d]", ctx.dataType);
    }
    ctx.reduceOp = kernelArg->opParam.reduceType;
    return CCU_SUCCESS;
}

static CcuResult InitResource(AllReduceExperimentalMesh1DContext& ctx)
{
    const auto* arg = ctx.arg;
    uint32_t channelIdx = 0;

    if (arg->rankSize < 1 || arg->rankSize > CCU_MAX_RANK_SIZE) {
        HCCL_ERROR(
            "[CcuAllReduceExperimentalMesh1D] invalid rankSize[%llu], valid range is [1, %llu].", arg->rankSize,
            static_cast<uint64_t>(CCU_MAX_RANK_SIZE) + 1);
        return CcuResult::CCU_E_INTERNAL;
    }
    if (arg->channelCount == 0) {
        HCCL_ERROR("[CcuAllReduceExperimentalMesh1D] channels is empty!");
        return CcuResult::CCU_E_INTERNAL;
    }
    if (arg->channelCount < arg->rankSize - 1) {
        HCCL_ERROR(
            "[CcuAllReduceExperimentalMesh1D] channelCount[%u] is less than required rankSize-1[%llu].",
            arg->channelCount, arg->rankSize - 1);
        return CcuResult::CCU_E_INTERNAL;
    }
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] channels.size: [%u]", arg->channelCount);
...
static CcuResult LoadArgs(AllReduceExperimentalMesh1DContext& ctx)
{
    const auto* arg = ctx.arg;
    uint32_t argId = 0;
...
static CcuResult PreSync(AllReduceExperimentalMesh1DContext& ctx)
{
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] AllReduceExperimentalMesh1D PreSync begin");
    const auto* arg = ctx.arg;
    ...
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] AllReduceExperimentalMesh1D PreSync end");
    return CCU_SUCCESS;
}

static CcuResult PostSync(AllReduceExperimentalMesh1DContext& ctx)
{
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] AllReduceExperimentalMesh1D post sync start");
    const auto* arg = ctx.arg;
    ...
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] AllReduceExperimentalMesh1D post sync end");
    return CCU_SUCCESS;
}

static CcuResult DoAllReduce(AllReduceExperimentalMesh1DContext& ctx)
{
    const auto* arg = ctx.arg;
    ...
    GroupReduce(
        ctx, arg->channels, arg->channelCount, reduceScatterDst, reduceScatterSrc, localSrc, ctx.goSize, ctx.dataType,
    ...
    GroupBroadcast(
        ctx, arg->channels, arg->channelCount, localDst, allGatherDst, allGatherSrc, ctx.goSize, GetCcuVersion());
...
// 主入口 Kernel 函数
// ============================================================================
CcuResult CcuAllReduceExperimentalMesh1DKernel(CcuKernelArg arg)
{
    auto* kernelArg = static_cast<CcuKernelArgAllReduceExperimentalMesh1D*>(arg);

    AllReduceExperimentalMesh1DContext ctx;
    ctx.arg = kernelArg;
    ctx.resourceAllocated = false;
    ...
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] AllReduceExperimentalMesh1D run");
    ...
    HCCL_INFO("[CcuAllReduceExperimentalMesh1D] AllReduceExperimentalMesh1D end");
