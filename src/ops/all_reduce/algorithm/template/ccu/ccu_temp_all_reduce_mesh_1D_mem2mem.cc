...
std::vector<CostModelParam> CcuTempAllReduceMeshMem2Mem1D::CalcCostCoeff(CalcCostCoeffParam param)
{
    ...
    float RTT = 0.000002;
    float Thbm = 0.000002;
    if (param.netType == CommTopo::COMM_TOPO_1DMESH) { // 全走mesh链路
    ...
    C = 0.0000006 * param.rankSize + 3 * RTT + log2R * Thbm;
...
HcclResult CcuTempAllReduceMeshMem2Mem1D::CalcRes(
    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG(
        "[CcuTempAllReduceMeshMem2Mem1D::CalcRes] notifyNumOnMainThread[%u] slaveThreadNum[%u]",
...
u64 CcuTempAllReduceMeshMem2Mem1D::GetThreadNum() const { return 1; }
...
HcclResult
CcuTempAllReduceMeshMem2Mem1D::FastLaunch(const OpParam& param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    (void)param;
...
void CcuTempAllReduceMeshMem2Mem1D::BuildTaskArgs(
...
void CcuTempAllReduceMeshMem2Mem1D::SaveSubmitInfo(
    const uint64_t inputAddr, const uint64_t outputAddr, const uint64_t token, const uint64_t scratchAddr,
    const uint64_t currentRankSliceInputOffset, const uint64_t currentRankSliceOutputOffset,
    const uint64_t normalSliceSize, const uint64_t lastSliceSize, const uint64_t mySliceSize,
    const uint64_t sliceOffset, const uint64_t isInputOutputEqual, const std::vector<uint64_t>& goSize,
    TemplateResource& templateResource) const
{
    ...
    (void)FillCachedArgs(
        ...
        currentRankSliceOutputOffset, normalSliceSize, lastSliceSize, mySliceSize, sliceOffset, isInputOutputEqual,
        goSize[0], goSize[1], goSize[2], goSize[3], buffInfo_.inBuffBaseOff, buffInfo_.outBuffBaseOff,
        buffInfo_.hcclBuffBaseOff);
    templateResource.submitInfos.push_back(submitInfo);
}

HcclResult CcuTempAllReduceMeshMem2Mem1D::KernelRun(
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    BuildTaskArgs(
        inputAddr, outputAddr, token, scratchAddr, currentRankSliceInputOffset, currentRankSliceOutputOffset,
        normalSliceSize, lastSliceSize, mySliceSize, sliceOffset, isInputOutputEqual, goSize, taskArgs);
    ...
    SaveSubmitInfo(
        inputAddr, outputAddr, token, scratchAddr, currentRankSliceInputOffset, currentRankSliceOutputOffset,
        normalSliceSize, lastSliceSize, mySliceSize, sliceOffset, isInputOutputEqual, goSize, templateResource);
