    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    if (topoInfo->level2Ubg) {
        HCCL_INFO(
            "[AllReduceAutoSelector][%s] ccu schedule is not supported with level2Ubg, reset to default.", __func__);
        return SelectorStatus::NOT_MATCH;
    }
    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    if (topoInfo->topoLevelNums > 1) {
        if (isDataTypeOrReduceTypeSpecial) {
            ...
            if (Level0AndLevel1Sym && topoInfo->deviceNumPerModule == DEVICE_NUM_PER_MODULE_8) {
                selectAlgName = "InsV2AllReduceOmniPipeUboe";
            } else if (!Level0AndLevel1Sym || topoInfo->netLayerDetails.localNetInsSizeOfLayer[1] == 1) {
            ...
            if (topoInfo->topoLevelNums == TOPO_LEVEL_NUM_3) {
                ...
                if (dataSize > AR_AICPU_1D_CROSS_SMALL_DATA_SIZE) {
                    selectAlgName = (dataSize > AR_AICPU_SEQUENCE_DATA_SIZE) ? "InsAllReduceSequenceMesh1DNhr" :
    ...
    bool isDataTypeOrReduceTypeSpecial) const
{
    ...
    if (isMeshNumEqualToClosNum && topoInfo->userRankSize <= MAX_RANK_NUM_FOR_CONCURRENT_ALGO) {
        if (isDataTypeOrReduceTypeSpecial) {
            selectAlgName = dataSize <= AR_AICPU_1D_64DATATYPE_DATA_SIZE ? "InsAllReduceMesh1DOneShot" :
    ...
    const TopoInfoWithNetLayerDetails* topoInfo, const OpParam& opParam, std::string& selectAlgName) const
{
    ...
    if (topoInfo->level0Topo == Level0Shape::MESH_1D) {
        if (isDataTypeOrReduceTypeSpecial) {
            ...
            selectAlgName = (isTwoLevelFlag && overSequenceDataThreshold) ? "InsAllReduceMesh1DTwoShotZAxisDetour" :
                                                                            "InsAllReduceMesh1DTwoShotMeshChunk";
        } else {
    ...
    const std::map<HcclCMDType, std::vector<HcclAlgoType>>& configAlgMap, std::string& selectAlgName) const
{
    ...
    if (topoInfo->level2Ubg) {
        HCCL_AIV_NOT_MATCH_LOG(
            opParam, HCCL_DEBUG, "[AllReduceAutoSelector][%s] aiv is not supported with level2Ubg, reset to default.",
            __func__);
        return SelectorStatus::NOT_MATCH;
    }
    ...
    if (dataSize > cclBufferSize * AIV_MAX_CCL_LOOP_NUM) {
        HCCL_AIV_NOT_MATCH_LOG(
            opParam, HCCL_DEBUG, "[AllReduceAutoSelector][%s] dataSize[%llu] too large for cclBufferSize [%llu]",
