...
HcclResult CcuTempAllReduceConcurrentMeshNHR::ProcessNHRStepInfo(HcclComm comm, u32 enableDieNum, u32 enableDieId,
...
HcclResult CcuTempAllReduceConcurrentMeshNHR::CalcMeshRes(HcclComm comm, const OpParam& param,
    const TopoInfoWithNetLayerDetails* topoInfo, CcuKernelInfo& meshKernelInfo)
{
    ...
    for (const auto& ch : myChannelDescs) {
        if (ch.channelProtocol == COMM_PROTOCOL_UBC_CTP) { channelDescs.push_back(ch); }
    }
...
HcclResult CcuTempAllReduceConcurrentMeshNHR::CalcNhrRes(HcclComm comm, const OpParam& param,
...
HcclResult CcuTempAllReduceConcurrentMeshNHR::CalcRes(HcclComm comm, const OpParam& param,
    const TopoInfoWithNetLayerDetails* topoInfo, AlgResourceRequest& resourceRequest)
{
    CHK_PRT_RET(topoInfo == nullptr,
        HCCL_ERROR("[CcuTempAllReduceConcurrentMeshNHR][CalcRes] topoInfo is nullptr"), HCCL_E_PARA);
    ...
    HCCL_INFO("[CcuTempAllReduceConcurrentMeshNHR][CalcRes] success, meshRank[%u], nhrRank[%u]",
...
HcclResult CcuTempAllReduceConcurrentMeshNHR::CalcDataSplit(const TemplateDataParams& templateDataParams,
    TemplateDataParams& meshParams, TemplateDataParams& nhrParams, u64& meshCount, u64& nhrCount) const
{
    ...
    meshCount = (portNum0 + portNum1) > 0 ? (dataCount * portNum0 / (portNum0 + portNum1)) : (dataCount / 2);
    ...
    HCCL_INFO("[CcuTempAllReduceConcurrentMeshNHR][CalcDataSplit] dataCount[%llu], meshCount[%llu], nhrCount[%llu]",
        dataCount, meshCount, nhrCount);
    return HCCL_SUCCESS;
}

HcclResult CcuTempAllReduceConcurrentMeshNHR::LaunchMeshKernel(const TemplateDataParams& meshParams,
    TemplateResource& templateResource, u64 baseInputAddr, u64 baseOutputAddr, uint64_t token,
    const LoopGroupConfig& config)
{
    ...
    CcuResult launchRet = HcommCcuKernelLaunch(templateResource.threads[0],
        templateResource.ccuKernels[0], taskArgs.data(), taskArgs.size());
...
HcclResult CcuTempAllReduceConcurrentMeshNHR::LaunchNhrKernel(const TemplateDataParams& nhrParams,
    TemplateResource& templateResource, u64 baseInputAddr, u64 baseOutputAddr, uint64_t token)
{
    ...
    std::vector<uint64_t> taskArgs = {inputAddr, outputAddr, token, isInputOutputEqual,
                                      die0Size, die1Size,
    ...
    CcuResult launchRet = HcommCcuKernelLaunch(templateResource.threads[MESH_THREAD_NUM_AR],
    ...
    CHK_RET(FillCachedArgs(nhrSubmit, inputAddr, outputAddr, token, isInputOutputEqual,
        die0Size, die1Size, die0SliceInfoVec[0][0].size, die1SliceInfoVec[0][0].size,
        die0SliceInfoVec[nhrRankSize - 1][0].size, die1SliceInfoVec[nhrRankSize - 1][0].size,
        nhrParams.buffInfo.inBuffBaseOff, nhrParams.buffInfo.outBuffBaseOff));
    templateResource.submitInfos.push_back(nhrSubmit);
    return HCCL_SUCCESS;
}

HcclResult CcuTempAllReduceConcurrentMeshNHR::KernelRun(const OpParam& param,
    const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    u64 nhrDataOff = templateDataParams.buffInfo.inBuffBaseOff + meshCount * dataTypeSize_;
    HCCL_INFO("[Concurrent][KernelRun] meshCount[%llu], nhrCount[%llu], nhrDataOff[%llu], meshOutputAddr[%llx], nhrOutputAddr[%llx]",
    ...
    HCCL_INFO("[CcuTempAllReduceConcurrentMeshNHR][KernelRun] done, meshCount[%llu], nhrCount[%llu]",
        meshCount, nhrCount);
    return HCCL_SUCCESS;
}
...
HcclResult CcuTempAllReduceConcurrentMeshNHR::FastLaunch(const OpParam& param,
    const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    ...
    // mesh 重放（submitInfos[0]）：argSize=8, offset idx 8/9
    {
        ...
        CcuResult ret = HcommCcuKernelLaunch(meshMain, submitInfos[0].kernelHandle,
    ...
    // nhr 重放（submitInfos[1]）：argSize=10, offset idx 10/11
    {
        ...
        CcuResult ret = HcommCcuKernelLaunch(nhrMain, submitInfos[1].kernelHandle,
            reinterpret_cast<void*>(args), 10);
        CHK_PRT_RET(ret != CCU_SUCCESS,
