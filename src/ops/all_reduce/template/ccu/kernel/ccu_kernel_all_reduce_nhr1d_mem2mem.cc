...
static CcuResult ParseKernelArg(AllReduceNHR1DContext& ctx, CcuKernelArgAllReduceNHR1D* kernelArg)
{
    ...
    HCCL_INFO(
        "[CcuKernelAllReduceNHR1D] kernelArg: rankId_[%u], axisId_[%u], axisSize_[%u], rankSize_[%u], localSize_[%u], "
...
static CcuResult PreSync(AllReduceNHR1DContext& ctx)
{
    ...
    for (uint32_t i = 0; i < arg->channelCount; i++) {
        ccu::WriteVariableWithNotify(
            arg->channels[i], ctx.output[ctx.localSize], OUTPUT_XN_ID, CKE_IDX_0, 1 << OUTPUT_XN_ID);
        ccu::WriteVariableWithNotify(
            arg->channels[i], ctx.token[ctx.localSize], TOKEN_XN_ID, CKE_IDX_0, 1 << TOKEN_XN_ID);
    }
...
static CcuResult DoWriteReduceSlice(
    AllReduceNHR1DContext& ctx, const u32& toRank, ccu::LocalAddr& src, ccu::RemoteAddr& dst, const u32& sendSliceIdx,
    u32 signalIndex)
{
    const auto* arg = ctx.arg;
    ...
    {
        ccu::WriteReduce(
            sendChannel, dst, src, sliceSize, ctx.dataType, ctx.reduceOp, ctx.localEvent, 1 << signalIndex);
    }
    CCU_IF(sliceSize == 0) { ccu::EventRecord(ctx.localEvent, 1 << signalIndex); }
    return CCU_SUCCESS;
}
    ...
    u32 signalIndex)
{
    ...
    ccu::Variable& sliceSize = ctx.axisId == 0 ? (islastSlice ? ctx.die0LastSliceSize : ctx.die0SliceSize) :
                                                 (islastSlice ? ctx.die1LastSliceSize : ctx.die1SliceSize);
    CCU_IF(sliceSize != 0) { ccu::Write(sendChannel, dst, src, sliceSize, ctx.localEvent, 1 << signalIndex); }
    CCU_IF(sliceSize == 0) { ccu::EventRecord(ctx.localEvent, 1 << signalIndex); }
    return CCU_SUCCESS;
}
...
static CcuResult DoLocalCopySlice(
    AllReduceNHR1DContext& ctx, ccu::LocalAddr& src, ccu::LocalAddr& dst, const u32& copySliceIdx, u32 signalIndex)
{
    bool islastSlice;
    // 添加 die1 偏移
    if (ctx.axisId == 1) {
    ...
    ccu::Variable& sliceSize = ctx.axisId == 0 ? (islastSlice ? ctx.die0LastSliceSize : ctx.die0SliceSize) :
                                                 (islastSlice ? ctx.die1LastSliceSize : ctx.die1SliceSize);
    CCU_IF(sliceSize != 0) { ccu::LocalCopy(dst, src, sliceSize, ctx.localEvent, 1 << signalIndex); }
    CCU_IF(sliceSize == 0) { ccu::EventRecord(ctx.localEvent, 1 << signalIndex); }
    return CCU_SUCCESS;
}
