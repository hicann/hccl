...
HcclResult CcuTempAlltoAllVMesh1D::CalcChannelRes(
    ...
    AlgResourceRequest& resourceRequest)
{
    ...
    HCCL_DEBUG(
        "[CcuTempAlltoAllVMesh1D::CalcRes] notifyNumOnMainThread[%u] slaveThreadNum[%u]",
...
HcclResult CcuTempAlltoAllVMesh1D::FastLaunch(const OpParam& param, const TemplateFastLaunchCtx& tempFastLaunchCtx)
{
    ...
    HCCL_INFO(
        "[CcuTempAlltoAllVMesh1D::FastLaunch]: inputPtr[%llu], outputPtr[%llu],srcOffset[%llu],"
        " dstOffset[%llu], rankSize[%llu], myRank[%lu]",
        PointerToAddr(tempFastLaunchCtx.buffInfo.inputPtr), PointerToAddr(tempFastLaunchCtx.buffInfo.outputPtr),
        args[3], args[4], args[5], myRank);
    ...
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    HCCL_INFO(
        "[CcuTempAllToAllVMesh1D] Run Init: myRank_[%d], dimSize[%llu], inputAddr[%llu],"
    ...
    CHK_RET(FillCachedArgs(
        subCommInfo, inputAddr, outputAddr, token, srcOffset, dstOffset, rankSize, myRank_, buffInfo_.inBuffBaseOff,
