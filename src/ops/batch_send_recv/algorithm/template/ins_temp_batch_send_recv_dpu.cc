    ...
    const OpParam& param, const TemplateDataParams& tempAlgParams, TemplateResource& templateResource)
{
    ...
    HCCL_INFO(
        "[InsTempBatchSendRecvDpu] recvRank_[%u] param.sendRecvRemoteRank[%u]", recvRank_, param.sendRecvRemoteRank);
    if (threadNum_ < 1) {
    ...
    if (channelIter == templateResource.channels.end() || channelIter->second.empty()) {
        HCCL_ERROR(
            "[InsTempBatchSendRecvDpu][KernelRun]my rank is [%d], receive rank [%u] channel not found!", myRank_,
            recvRank_);
        return HCCL_E_INTERNAL;
    }
    ...
    if (sendRecvChannel_.locationType == EndpointLocType::ENDPOINT_LOC_TYPE_HOST) {
        if (tempAlgParams.opType == BatchSendRecvOpType::SEND) {
            ...
            DataSlice localCclBuffer(
                ...
                tempAlgParams.buffInfo.hcclBuffBaseOff + myRank_ * hcclbuffBlockMemSize_, processSize_,
                count_);                                              // copy到对应本rank划分的cclbuffer
        ...
        if (tempAlgParams.opType == BatchSendRecvOpType::RECV) {
            ...
            DataSlice localCclBuffer(
                tempAlgParams.buffInfo.hcclBuff.addr,
                tempAlgParams.buffInfo.hcclBuffBaseOff + recvRank_ * hcclbuffBlockMemSize_, processSize_,
                count_); // 从对端rank对应划分的cclbuffer接收
        ...
        if (tempAlgParams.opType == BatchSendRecvOpType::SEND) {
            ...
            DataSlice inputBuffer(
                tempAlgParams.buffInfo.inputPtr, tempAlgParams.buffInfo.inBuffBaseOff, processSize_, count_);
            DataSlice remoteCclBuffer(
                tempAlgParams.buffInfo.outputPtr,
                tempAlgParams.buffInfo.hcclBuffBaseOff + myRank_ * hcclbuffBlockMemSize_, processSize_,
                count_); // 发送到对端对应本rank划分的cclbuffer
            // 发送
            ...
            CHK_PRT_RET(
                SendWrite(sendInfo, thread_), HCCL_ERROR("[InsTempBatchSendRecvDpu][KernelRun]Aicpu Run Send failed"),
    ...
    const OpParam& param, const TemplateDataParams& tempAlgParams, TemplateResource& templateResource)
{
    ...
    if (HcommSendRequest(
        ...
        != 0) {
        HCCL_ERROR("[InsTempBatchSendRecvDpu] HcommSendRequest failed");
        return HCCL_E_INTERNAL;
    }
    HCCL_INFO("[InsTempBatchSendRecvDpu] HcommSendRequest run over, sendMsgId[%u]", sendMsgId);
    // 等待DPU数据传输，然后回写结果回来
    ...
    if (HcommWaitResponse(reinterpret_cast<uint64_t>(templateResource.dpu2NpuShmemPtr), recvData, 0, &recvMsgId) != 0) {
        HCCL_ERROR("[InsTempBatchSendRecvDpu] HcommWaitResponse failed");
        return HCCL_E_INTERNAL;
    }

    // 将执行模式转换回到batch
    if (HcommBatchModeStart(param.algTag) != HCCL_SUCCESS) {
        HCCL_ERROR("[InsTempBatchSendRecvDpu] failed set eager mode, tag is %s.", param.algTag);
        return HCCL_E_INTERNAL;
    }
    HCCL_INFO("[InsTempBatchSendRecvDpu] HcommWaitResponse run over, recvMsgId[%u]", recvMsgId);

    if (recvMsgId != sendMsgId) {
        HCCL_ERROR("[InsTempBatchSendRecvDpu] recvMsgId[%u] not equal to sendMsgId[%u]", recvMsgId, sendMsgId);
        return HCCL_E_INTERNAL;
    }
    return HCCL_SUCCESS;
}
    ...
    const std::vector<std::vector<uint32_t>>& subCommRanks)
{
    ...
    if (channelIter == channels.end() || channelIter->second.empty()) {
        HCCL_ERROR(
            "[InsTempBatchSendRecvDpu] [DPUKernelRun] my rank is [%d], send rank [%u] channel not found!", myRank,
            sendRank);
        return HCCL_E_INTERNAL;
    }
    ...
    if (tempAlgParam.opType == BatchSendRecvOpType::RECORD) {
        ...
        DataSlice localCclBuffer(
            tempAlgParam.buffInfo.hcclBuff.addr, tempAlgParam.buffInfo.hcclBuffBaseOff + myRank * hcclbuffBlockMemSize_,
            tempAlgParam.sliceSize);
        DataSlice remoteCclBuffer(
            tempAlgParam.buffInfo.outputPtr, tempAlgParam.buffInfo.hcclBuffBaseOff + myRank * hcclbuffBlockMemSize_,
            tempAlgParam.sliceSize); // ccl
        // 发送
        ...
        for (int i = 0; i < sliceNum; i++) {
            ...
            CHK_RET(static_cast<HcclResult>(HcommWriteWithNotifyNbiOnThread(
                0, sendChannel.handle, dst, src, srcSlice.size_, NOTIFY_IDX_DATA_SIGNAL)));
        }
        return HCCL_SUCCESS;
        ...
        DataSlice remoteInputBuffer(
            tempAlgParam.buffInfo.inputPtr, tempAlgParam.buffInfo.inBuffBaseOff, tempAlgParam.sliceSize,
            tempAlgParam.count);
        DataSlice localCclBuffer(
            tempAlgParam.buffInfo.hcclBuff.addr, 0, tempAlgParam.sliceSize,
