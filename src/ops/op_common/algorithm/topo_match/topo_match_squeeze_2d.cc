    ...
    AlgHierarchyInfoForAllLevel& algHierarchyInfo) const
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[TopoMatchSqueeze2D][TopoForLayer0] layer0 topoInstNum [%u] != 1, only Mesh1D supported.", topoInstNum),
    ...
    CHK_PRT_RET(
        topoInstNumLayer1 != NET_INST_NUM_1,
        HCCL_ERROR("[TopoMatchSqueeze2D][TopoForLayer0] layer1 topoInstNum [%u] != 1.", topoInstNumLayer1),
        HcclResult::HCCL_E_NOT_SUPPORT);
    ...
    HCCL_DEBUG(
        "[TopoMatchSqueeze2D] Rank [%u], all [%u] ranks in layer1: [%s]", myRank, rankNumLayer1,
        PrintCArray<uint32_t>(ranksLayer1, rankNumLayer1).c_str());
    ...
    HCCL_DEBUG(
        "[TopoMatchSqueeze2D][TopoForLayer0] Rank [%u], layer0 group: [%s]", myRank,
        PrintCArray<uint32_t>(ranksInfos0.data(), static_cast<u32>(ranksInfos0.size())).c_str());
#endif
    ...
    AlgHierarchyInfoForAllLevel& algHierarchyInfo) const
{
    ...
    HCCL_DEBUG(
        "[TopoMatchSqueeze2D][TopoForLayer1] Rank [%u], all [%u] ranks in layer2: [%s]", myRank, rankNum,
    ...
    HCCL_DEBUG(
        "[TopoMatchSqueeze2D][TopoForLayer1] Rank [%u], layer1 group: [%s]", myRank,
        PrintCArray<uint32_t>(rankVecLayer1WithSameIdx.data(), static_cast<u32>(rankVecLayer1WithSameIdx.size()))
            .c_str());
#endif
    return HcclResult::HCCL_SUCCESS;
}
...
HcclResult TopoMatchSqueeze2D::MatchTopo(
    const HcclComm comm, TopoInfoWithNetLayerDetails* topoInfo, AlgHierarchyInfoForAllLevel& algHierarchyInfo)
{
    ...
    CHK_PRT_RET(
        topoInfo->topoLevelNums != EXPECTED_TOPO_LEVEL_NUM_3,
        HCCL_ERROR(
            "[TopoMatchSqueeze2D][MatchTopo] topoLevelNums [%u] != 3, only 3-level topo supported.",
    ...
    HCCL_DEBUG(
        "[TopoMatchSqueeze2D] Rank [%u], netLayers [%u][%s]", myRank, layerNum,
        PrintCArray<uint32_t>(netLayers, layerNum).c_str());

    CHK_PRT_RET(
        layerNum < EXPECTED_TOPO_LEVEL_NUM_3,
    ...
    HCCL_INFO(
        "[TopoMatchSqueeze2D] Rank [%u], [%u] pods, ranksize on each pod: [%s]", myRank, listSize,
        PrintCArray<uint32_t>(instSizeList, listSize).c_str());

    bool isSymmetric = CheckVecElementAllSame(instSizeList, listSize);
    CHK_PRT_RET(
        !isSymmetric, HCCL_ERROR("[TopoMatchSqueeze2D][MatchTopo] Only symmetric topology supported."),
        HcclResult::HCCL_E_NOT_SUPPORT);
