...
void CalcGatherStepDataAndOffset(GatherSliceContext& ctx)
{
    ...
    for (u64 rs = 0; rs < ctx.rankSize; rs++) {
        if (xyGtZ) {
            ctx.outerStepNum = CalAllgatherDataSize2D(
                ctx.zGDS[rs].data(), ctx.xyGDS[rs].data(), ctx.zB, ctx.xyB, ctx.zRankSize, xyRankSize,
                ctx.perLoop[rs].size, ctx.maxStepNum, ctx.engine);
        } else {
            ctx.outerStepNum = CalAllgatherDataSize2D(
                ctx.xyGDS[rs].data(), ctx.zGDS[rs].data(), ctx.xyB, ctx.zB, xyRankSize, ctx.zRankSize,
                ctx.perLoop[rs].size, ctx.maxStepNum, ctx.engine);
        }
        for (u64 i = 0; i < ctx.outerStepNum; i++) {
            if (ctx.yGeX) {
                ctx.innerStepNum = CalAllgatherDataSize2D(
                    ctx.xGDS[rs][i].data(), ctx.yGDS[rs][i].data(), ctx.xB, ctx.yB, ctx.xRankSize, ctx.yRankSize,
                    ctx.xyGDS[rs][i], ctx.maxStepNum, ctx.engine);
            } else {
                ctx.innerStepNum = CalAllgatherDataSize2D(
                    ctx.yGDS[rs][i].data(), ctx.xGDS[rs][i].data(), ctx.yB, ctx.xB, ctx.yRankSize, ctx.xRankSize,
                    ctx.xyGDS[rs][i], ctx.maxStepNum, ctx.engine);
            }
        }
        if (ctx.yGeX) {
            if (ctx.innerStepNum > 1) {
                ctx.xInCornerStep = ctx.innerStepNum - 1;
            }
        } else {
            if (ctx.innerStepNum > 1) {
                ctx.yInCornerStep = ctx.innerStepNum - 1;
            }
        }
        if (xyGtZ) {
            CalAllgather2DOffset(
                ctx.zGOff[rs].data(), ctx.xyGOff[rs].data(), ctx.outerStepNum, ctx.zRankSize, xyRankSize,
                ctx.zGDS[rs].data(), ctx.xyGDS[rs].data());
        } else {
            CalAllgather2DOffset(
                ctx.xyGOff[rs].data(), ctx.zGOff[rs].data(), ctx.outerStepNum, xyRankSize, ctx.zRankSize,
                ctx.xyGDS[rs].data(), ctx.zGDS[rs].data());
        }
    }
    if (xyGtZ) {
        if (ctx.outerStepNum > 1) {
            ctx.zCornerStep = ctx.outerStepNum - 1;
        }
    } else {
        if (ctx.outerStepNum > 1) {
            ctx.xyCornerStep = ctx.outerStepNum - 1;
        }
    }
    HCCL_INFO(
        "[CalcGatherOmniPipeSliceInfo] xInCornerStep=[%d],yInCornerStep=[%d],zConnerStep=[%d]", ctx.xInCornerStep,
        ctx.yInCornerStep, ctx.zCornerStep);
}
...
void CalcGatherXY2DOffset(GatherSliceContext& ctx)
{
    for (u64 rs = 0; rs < ctx.rankSize; rs++) {
        for (u64 osn = 0; osn < ctx.outerStepNum; osn++) {
            if (ctx.yGeX) {
                CalAllgather2DOffset(
                    ctx.xGOff[rs][osn].data(), ctx.yGOff[rs][osn].data(), ctx.innerStepNum, ctx.xRankSize,
                    ctx.yRankSize, ctx.xGDS[rs][osn].data(), ctx.yGDS[rs][osn].data());
            } else {
                CalAllgather2DOffset(
                    ctx.yGOff[rs][osn].data(), ctx.xGOff[rs][osn].data(), ctx.innerStepNum, ctx.yRankSize,
                    ctx.xRankSize, ctx.yGDS[rs][osn].data(), ctx.xGDS[rs][osn].data());
            }
        }
    }
}
...
OmniPipeSliceInfo CalcGatherOmniPipeSliceInfo(OmniPipeSliceParam& omniPipeSliceParam)
{
    ...
    HCCL_INFO(
        "[CalcGatherOmniPipeSliceInfo] xRankSize=[%llu],yRankSize=[%llu],zRankSize=[%llu],", ctx.xRankSize,
        ctx.yRankSize, ctx.zRankSize);
    HCCL_INFO("[CalcGatherOmniPipeSliceInfo] xB=[%f],yB=[%f],zB=[%f],xyB=[%f]", ctx.xB, ctx.yB, ctx.zB, ctx.xyB);
    HCCL_INFO(
        "[CalcGatherOmniPipeSliceInfo] xAxis=[%llu],yAxis=[%llu],zAxis=[%llu],rankid=[%llu]", ctx.xAxis, ctx.yAxis,
        ctx.zAxis, rankid);
