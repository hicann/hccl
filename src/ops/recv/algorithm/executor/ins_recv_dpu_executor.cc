    ...
    TopoInfoWithNetLayerDetails* topoInfo, AlgHierarchyInfoForAllLevel& algHierarchyInfo, const AlgAttrs& algAttrs)
{
    (void)algAttrs;
    myRank_ = topoInfo->userRank;
    HCCL_DEBUG("[InsRecvDpuExecutor][CalcAlgHierarchyInfoV2][%d] Start.", myRank_);
    CHK_PRT_RET(
        topoInfo->userRankSize == 0,
        HCCL_ERROR("[InsRecvDpuExecutor][CalcAlgHierarchyInfoV2] Rank [%d], rankSize is 0.", myRank_),
        HcclResult::HCCL_E_PARA);
    ...
    HCCL_DEBUG("[InsRecvDpuExecutor][CalcAlgHierarchyInfoV2][%d] Success.", myRank_);
    return HcclResult::HCCL_SUCCESS;
}
...
template <typename InsAlgTemplate>
HcclResult
InsRecvDpuExecutor<InsAlgTemplate>::Orchestrate(const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
    ...
    // 选择template
    std::shared_ptr<InsAlgTemplate> algTemplateDpu
...
REGISTER_ALG_ATTRS(
    DpuRecvSoleHost, topo.isHostDpuOnly = true;
