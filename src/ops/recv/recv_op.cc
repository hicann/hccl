    ...
    void* recvBuf, uint64_t count, HcclDataType dataType, uint32_t srcRank, const HcclComm comm, aclrtStream stream)
{
    ...
    CHK_RET_AND_PRINT_IDE(
        RecvExec(recvBuf, count, dataType, srcRank, comm, stream, rankSize, OpMode::OPBASE, tag), tag.c_str());
    ...
    const char* tag, void** streams, size_t streamCount, void* scratchMemAddr, uint64_t scratchMemSize)
{
    ...
    // 参数获取和校验
    CHK_PRT_RET(
        count == 0, HCCL_WARNING("[HcclRecvGraphMode] input count is 0, return recv success"),
    ...
    // 执行Recv
    CHK_RET_AND_PRINT_IDE(
        RecvExec(recvBuf, count, dataType, srcRank, comm, stream, rankSize, OpMode::OFFLOAD, opTag, resPack),
        opTag.c_str());
    ...
    u32& rankSize, u32& userRank, std::string& tag)
{
    ...
    CHK_PRT_RET(
        userRank == srcRank, HCCL_ERROR("[HcclRecv] srcRank cannot be equal to self."), HcclResult::HCCL_E_NOT_SUPPORT);
    if (tag.empty()) {
    ...
    const ResPackGraphMode& resPack)
{
    ...
    if (rankSize == 1) {
        HCCL_WARNING(
            "[RecvExec][%s][%s] ranksize == 1, enter SingleRankProc", tagTemp.c_str(),
    ...
    const std::string& opName, bool forceLog)
{
    if (forceLog || GetExternalInputHcclEnableEntryLog()) {
        ...
        s32 ret = snprintf_s(
            ...
            "tag[%s], recvBuf[%p], count[%llu], dataType[%s], srcRank[%u], streamId[%d], deviceId[%d]", tag.c_str(),
            recvBuf, count, GetDataTypeEnumStr(dataType).c_str(), srcRank, streamId, deviceId);
