...
static CcuResult
ParseKernelArg(GatherOmniPipeMesh1DMem2MemContext& ctx, CcuKernelArgGatherOmniPipeMesh1DMem2Mem* kernelArg)
{
    ctx.arg = kernelArg;
...
static CcuResult PreSync(GatherOmniPipeMesh1DMem2MemContext& ctx)
{
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        ccu::WriteVariableWithNotify(
            ctx.arg->channels[i], ctx.input[ctx.rankId], INPUT_XN_ID, CKE_IDX_0, 1 << INPUT_XN_ID);
        ccu::WriteVariableWithNotify(
...
static CcuResult DoGather(GatherOmniPipeMesh1DMem2MemContext& ctx)
{
    ...
    for (uint64_t rankIdx = 0; rankIdx < ctx.rankSize; rankIdx++) {
        uint64_t rankMask = 1ULL << rankIdx;
        ...
        CCU_IF(ctx.sliceSize != 0)
        {
            if (ctx.rankId != rankIdx) {
                ccu::Read(
                    ...
                    ctx.event, rankMask);
                HCCL_DEBUG(
                    "[%s]channelId[%u] rankIdx[%u] inputMem[%llu] sliceSize[%u]", __func__, channelId, rankIdx,
                    ctx.inputMem[rankIdx], ctx.sliceSize);
                channelId++;
            } else {
                ccu::EventRecord(ctx.event, rankMask);
            }
    ...
    ccu::EventWait(ctx.event, (1ULL << ctx.rankSize) - 1);
    return CCU_SUCCESS;
}
...
static CcuResult DoRepeatGather(GatherOmniPipeMesh1DMem2MemContext& ctx)
{
    ...
    CCU_IF(ctx.ifNewRoot == true) { CCU_CHK_RET(DoGather(ctx)); }
...
CcuResult CcuGatherOmniPipeMesh1DMem2MemKernel(CcuKernelArg arg)
{
    ...
    HCCL_DEBUG("[CcuGatherOmniPipeMesh1DMem2Mem] GatherOmniPipeMesh1DMem2Mem end");
