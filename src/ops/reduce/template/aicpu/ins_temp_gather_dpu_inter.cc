...
HcclResult InsTempGatherDpuInter::CalcRes(HcclComm comm, const OpParam &param,
...
HcclResult InsTempGatherDpuInter::LocalDataCopy(
                ...
                if (sendSize > 0 && recvSize > 0) {
                    if (txChannel[0].remoteRank == rxChannel[0].remoteRank) {
                        ...
                        CHK_PRT_RET(SendWrite(sendInfo),
                            ...
                            HcclResult::HCCL_E_INTERNAL);
                        HCCL_INFO("[InsTempGatherDpuInter][RunNHR]SendWrite src addr %p, dst addr %p",
                            tempAlgParams.buffInfo.hcclBuff.addr, sendCclBuffAddr);
...
HcclResult InsTempGatherDpuInter::PostLocalCopy(
    const TemplateDataParams &tempAlgParams, const TemplateResource &templateResource)
{
