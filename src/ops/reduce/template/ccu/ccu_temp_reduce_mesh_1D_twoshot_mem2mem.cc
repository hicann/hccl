...
void CcuTempReduceMesh1DTwoShotMem2Mem::SetRoot(u32 root)
{
    ...
    HCCL_INFO(
        "[CcuTempReduceMesh1DTwoShotMem2Mem][SetRoot] myRank_ [%u], set root [%u] subCommRoot [%u]", mySubCommRank_,
        root, mySubCommRoot_);
}

HcclResult CcuTempReduceMesh1DTwoShotMem2Mem::CalcRes(
...
HcclResult CcuTempReduceMesh1DTwoShotMem2Mem::KernelRun(
    const OpParam& param, const TemplateDataParams& templateDataParams, TemplateResource& templateResource)
{
    ...
    HCCL_INFO(
        "[CcuTempReduceMesh1DTwoShotMem2Mem::KernelRun] TaskArgs: inputAddr[%llu], outputAddr[%llu], "
        "scratchAddr[%llu], normalSliceSize[%llu], lastSliceSize[%llu], repeatNumVar[%llu], "
        "inputRepeatStride[%llu], outputRepeatStride[%llu]",
        inputAddr, outputAddr, scratchAddr, normalSliceSize, lastSliceSize, repeatNumVar, inputRepeatStride,
        outputRepeatStride);

    std::vector<uint64_t> taskArgs
        = {inputAddr,         outputAddr,         scratchAddr, token,     normalSliceSize, lastSliceSize, repeatNumVar,
           inputRepeatStride, outputRepeatStride, goSize[0],   goSize[1], goSize[2],       goSize[3]};
...
HcclResult CcuTempReduceMesh1DTwoShotMem2Mem::SubmitKernelInfo(
...
u64 CcuTempReduceMesh1DTwoShotMem2Mem::GetThreadNum() const { return 1; }
