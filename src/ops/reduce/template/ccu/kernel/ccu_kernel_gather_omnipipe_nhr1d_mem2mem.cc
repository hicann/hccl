...
static CcuResult
ParseKernelArg(GatherOmniPipeNHR1DMem2MemContext& ctx, CcuKernelArgGatherOmniPipeNHR1DMem2Mem* kernelArg)
{
    ctx.arg = kernelArg;
...
static CcuResult InitResource(GatherOmniPipeNHR1DMem2MemContext& ctx)
{
    ...
    HCCL_DEBUG(
        "[CcuGatherOmniPipeNHR1DMem2Mem] channels.size:[%u] localSize:[%u]", ctx.arg->channelCount, ctx.localSize);
...
static CcuResult PreSync(GatherOmniPipeNHR1DMem2MemContext& ctx)
{
    for (uint32_t i = 0; i < ctx.arg->channelCount; i++) {
        ccu::WriteVariableWithNotify(
            ctx.arg->channels[i], ctx.input[ctx.myRankIdx], INPUT_XN_ID, CKE_IDX_0, 1 << INPUT_XN_ID);
        ccu::WriteVariableWithNotify(ctx.arg->channels[i], ctx.output, SCRATCH_XN_ID, CKE_IDX_0, 1 << SCRATCH_XN_ID);
        ccu::WriteVariableWithNotify(
...
static CcuResult DoGatherOmniPipeNHRSingleStep(GatherOmniPipeNHR1DMem2MemContext& ctx, const NHRStepInfo& nhrStepInfo)
{
    ...
    if (recvSliceIdxList.size() != 0) {
        ...
        for (u32 i = 0; i < recvSliceIdxSize; i++) {
            ...
            CCU_IF(ctx.sliceSize != 0) { ccu::Read(recvChannel, dst, src, ctx.sliceSize, ctx.event, 1 << i); }
            CCU_IF(ctx.sliceSize == 0) { ccu::EventRecord(ctx.event, 1 << i); }
        }
