...

{
    ...
    if (ctx.outputDataType == HcclDataType::HCCL_DATA_TYPE_RESERVED) {
        ...
        HCCL_DEBUG("[CcuKernelReduceMesh1D] outputDataType is [INVALID], set outputDataType to[%d]", ctx.dataType);
    }
...
static CcuResult DoRepeatReduce(ReduceMesh1DContext& ctx)
{
    ...
    GroupReduce(
        ctx, arg->channels, arg->channelCount, dst, remoteSrc, localSrc, ctx.groupOpSize, ctx.dataType,
