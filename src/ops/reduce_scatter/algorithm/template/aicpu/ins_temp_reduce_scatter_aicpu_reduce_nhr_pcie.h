...
#ifndef INS_TEMP_REDUCE_SCATTER_AICPU_REDUCE_NHR_PCIE_H
#define INS_TEMP_REDUCE_SCATTER_AICPU_REDUCE_NHR_PCIE_H
...
class InsTempReduceScatterAicpuReduceNHRPcie : public InsAlgTemplateBase {
public:
    InsTempReduceScatterAicpuReduceNHRPcie() = default;
    explicit InsTempReduceScatterAicpuReduceNHRPcie(
        const OpParam& param, const u32 rankId, // 传通信域的rankId，userRank
        const std::vector<std::vector<u32>>& subCommRanks);
    ~InsTempReduceScatterAicpuReduceNHRPcie() override;
...
private:
    ...
    u64 dataTypeSize_{0};
    u64 currSliceSize_{0};   // 当前sliceIdx对应的数据大小，尾块时为tailSize
    u64 currSliceCount_{0};  // 当前sliceIdx对应的数据元素个数
    u64 hcclBuffBaseOff_{0}; // 当前使用的hcclBuff基础偏移
};

} // namespace ops_hccl

#endif // INS_TEMP_REDUCE_SCATTER_AICPU_REDUCE_NHR_PCIE_H
