...
std::vector<CostModelParam> InsTempReduceScatterOrderPreservedGroup::CalcCostCoeff(CalcCostCoeffParam param)
{
    float A = 0.0f;
    float B = 0.0f;
    float C = 0.0f;
    ...
    const TemplateDataParams& tempAlgParams, const std::vector<ThreadHandle>& threads)
{
    ...
    if (sliceSize == 0) {
        HCCL_DEBUG(
            "[InsTempReduceScatterOrderPreservedGroup][PreLocalCopy] myAlgRank[%u] sliceSize is 0, skip.", myAlgRank);
        return HCCL_SUCCESS;
    }
    ...
    HCCL_INFO(
        "[InsTempReduceScatterOrderPreservedGroup][PreLocalCopy] myAlgRank[%u], sliceSize[%llu], srcOffset[%llu], "
    ...
    const TemplateDataParams& tempAlgParams)
{
    ...
    HCCL_INFO("[InsTempReduceScatterOrderPreservedGroup][RunAllToAll] End");
    return HCCL_SUCCESS;
}
...
{
    HCCL_INFO(
        "[InsTempReduceScatterOrderPreservedGroup][RunLocalReduce] Start, deterministicStrict[%d], "
        "templateRankSize[%u]",
        deterministicStrict_, templateRankSize_);

    if (templateRankSize_ <= 1) {
        HCCL_INFO("[InsTempReduceScatterOrderPreservedGroup][RunLocalReduce] Skip for single rank");
        return HCCL_SUCCESS;
    }
    ...
    HCCL_INFO("[InsTempReduceScatterOrderPreservedGroup][RunLocalReduce] myAlgRank[%u]", myAlgRank);
    ...
    while (remainingBlocks > 1) {
        ...
        HCCL_INFO(
            "[InsTempReduceScatterOrderPreservedGroup][RunLocalReduce] Step[%u]: remainingBlocks[%u], M[%u]", step,
            remainingBlocks, M);
        ...
        for (u32 srcVirtualIdx = M; srcVirtualIdx < remainingBlocks; srcVirtualIdx++) {
            ...
            HCCL_INFO(
                "[InsTempReduceScatterOrderPreservedGroup][RunLocalReduce] Step[%u]: virtualIdx[%u]->[%u], "
                "peerRank[%u]->[%u], "
                "offset[%llu]->[%llu]",
    ...
    HCCL_INFO(
        "[InsTempReduceScatterOrderPreservedGroup][RunLocalReduce] End, total steps[%u], final data at virtualIdx[0] "
        "(myAlgRank[%u])",
        step, myAlgRank);
    return HCCL_SUCCESS;
}
    ...
    const TemplateDataParams& tempAlgParams, const std::vector<ThreadHandle>& threads)
{
    ...
    if (sliceSize == 0) {
        HCCL_DEBUG(
            "[InsTempReduceScatterOrderPreservedGroup][PostCopy] myAlgRank[%u] sliceSize is 0, skip.", myAlgRank);
        return HCCL_SUCCESS;
    }
    ...
    HCCL_INFO(
        "[InsTempReduceScatterOrderPreservedGroup][PostCopy] myAlgRank[%u], sliceSize[%llu], srcOffset[%llu], "
        "dstOffset[%llu]",
