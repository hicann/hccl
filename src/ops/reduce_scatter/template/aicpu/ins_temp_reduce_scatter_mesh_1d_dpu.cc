...
// ! 已编码完成
HcclResult InsTempReduceScatterMesh1dDpu::CalcRes(
...
// ! 基本编码完成，剩余数据序列化
HcclResult InsTempReduceScatterMesh1dDpu::KernelRun(
...
HcclResult InsTempReduceScatterMesh1dDpu::DPUKernelRun(
...
// ! 已完成编码，待与堂植确认加法序问题
HcclResult InsTempReduceScatterMesh1dDpu::PostLocalReduce(
    const OpParam& param, const TemplateDataParams& tempAlgParams, const std::vector<ThreadHandle>& threads)
{
    ...
    for (u32 repeatIdx = 0; repeatIdx < tempAlgParams.repeatNum; repeatIdx++) {
        u64 dataTypeSize = HCCL_SIZE_TABLE[dataType_];
        ...
        dstSlice = DataSlice(
            ...
            tempAlgParams.buffInfo.outBuffBaseOff + repeatIdx * tempAlgParams.outputRepeatStride, processSize_, count_);
        CHK_RET(static_cast<HcclResult>(LocalCopy(threads[0], srcSlice, dstSlice)));
    }
    return HcclResult::HCCL_SUCCESS;
}
