...
static uint32_t GetSignalIndex(const int signalBit) { return static_cast<uint32_t>(signalBit) / BIT_NUM_PER_CKE; }
...
static CcuResult DoRepeatSendRecvSlices(
    ReduceScatterNhrMem2Mem1DMultiJettyContext& ctx, const uint32_t& toRank, ccu::LocalAddr& src, ccu::RemoteAddr& dst)
{
    ...
    CCU_WHILE(ctx.repeatNumVarTemp != UINT64_MAX)
    {
        ...
        CCU_IF(ctx.sliceOneJettySize != 0)
        {
            for (uint32_t jettyId = 0; jettyId < ctx.portNum - 1; jettyId++) {
                ccu::WriteReduce(
                    arg->channels[ctx.rank2ChannelIdx.at(toRank)], tempDst, tempSrc, ctx.sliceOneJettySize,
                    ctx.dataType, ctx.reduceOp, ctx.jettyEvent[jettyId], 1);
                tempDst.addr += ctx.sliceOneJettySize;
                tempSrc.addr += ctx.sliceOneJettySize;
            }
        }
        CCU_IF(ctx.sliceLastJettySize == 0) { ccu::EventRecord(ctx.jettyEvent[ctx.portNum - 1], 1); }
        CCU_IF(ctx.sliceLastJettySize != 0)
        {
            uint32_t jettyId = ctx.portNum - 1;
            ccu::WriteReduce(
                arg->channels[ctx.rank2ChannelIdx.at(toRank)], tempDst, tempSrc, ctx.sliceLastJettySize, ctx.dataType,
                ctx.reduceOp, ctx.jettyEvent[jettyId], 1);
        }
...
static CcuResult DoRepeatReduceScatterNHRSingleStep(
    ReduceScatterNhrMem2Mem1DMultiJettyContext& ctx, const NHRStepInfo& nhrStepInfo,
...
static CcuResult DoRepeatReduceScatter(ReduceScatterNhrMem2Mem1DMultiJettyContext& ctx)
{
    ...
    CCU_WHILE(ctx.repeatNumVar != UINT64_MAX)
    {
        ...
        CCU_IF(ctx.sliceSize != 0) { ccu::LocalCopy(ctx.localDst, ctx.localSrc, ctx.sliceSize, ctx.event, 1); }
