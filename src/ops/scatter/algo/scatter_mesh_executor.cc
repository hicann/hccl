...
HcclResult ScatterMeshExecutor::CalcResRequest(
...
HcclResult ScatterMeshExecutor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    CHK_PRT_RET(
        bRet, HCCL_ERROR("[ScatterMeshExecutor][KernelRun]tag[%s],comm level0 is empty", tag_.c_str()),
        HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ScatterMeshExecutor][KernelRun]GetSubRootForScatter failed, "
            "userRank[%u], root[%u], subRoot[%u]",
            topoInfo_->userRank, param.root, subRoot),
        HCCL_E_INTERNAL);
    HCCL_DEBUG(
        "[ScatterMeshExecutor][KernelRun]GetSubRootForScatter, userRank[%u], root[%u], subRoot[%u]",
        topoInfo_->userRank, param.root, subRoot);
    CHK_RET(KernelRunLevel1(
        execMem.inputMem, execMem.count, param.DataDes.dataType, commIndex, param.root, subRoot, COMM_LEVEL1, thread_));
    ...
    std::unique_ptr<AlgTemplateBase> level0TempAlg
        = AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_SCATTER_MESH);
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ScatterMeshExecutor][KernelRun]rootRankLevel0[%u] is invalid, userRank[%u], subRoot[%u]", rootRankLevel0,
            topoInfo_->userRank, subRoot),
        HCCL_E_INTERNAL);

    CHK_RET(level0TempAlg->Prepare(
        scatterMeshInput, scatterMeshOutput, execMem.inputMem, execMem.count, param.DataDes.dataType, thread_,
    ...
    CHK_RET(
        static_cast<HcclResult>(HcommLocalCopyOnThread(thread_, execMem.outputMem.addr, src, execMem.outputMem.size)));
    return HCCL_SUCCESS;
}
