...
HcclResult InsTempScatterMesh1D::CalcRes(HcclComm comm, const OpParam &param,
    ...
    const OpParam &param, const TemplateDataParams &tempAlgParams, TemplateResource &templateResource)
{
    ...
    CHK_PRT_RET(templateResource.threads.empty(), HCCL_ERROR("[InsTempScatterMesh1D][KernelRun] threads is empty"),
        HCCL_E_INTERNAL);
    u32 myAlgRank = 0;
...
HcclResult InsTempScatterMesh1D::RunMesh(const std::map<u32, std::vector<ChannelInfo>> &channels,
    const std::vector<ThreadHandle> &threads, const TemplateDataParams &tempAlgParams)
{
    ...
    const u32 dataTypeSize = DATATYPE_SIZE_TABLE[dataType_];
    CHK_RET(GetAlgRank(myRank_, subCommRanks_[0], myAlgRank));
    HCCL_DEBUG("[InsTempScatterMesh1D][RunMesh] myRank[%d], myAlgRank[%d], channels size[%d]", myRank_, myAlgRank,
        channels.size());
    if (root_ == u32(myRank_)) {
        ...
        for (u32 algRank = 0; algRank < subCommRanks_[0].size(); algRank++) {
            ...
            // 需要根据channel切分数据
            CHK_RET(CalcDataSplitByPortGroup(curCount, dataTypeSize, linkSends, elemCountOut_, sizeOut_, elemOffset_));
            // 循环每个channel发送数据
            for (u32 channelIdx = 0; channelIdx < channelsPerRank_; channelIdx++) {
                ...
                for (u32 r = 0; r < tempAlgParams.repeatNum; r++) {
                    ...
                    HCCL_DEBUG("[InsTempScatterMesh1D][RunMesh] srcOffset[%d], tempAlgParams.buffInfo.inputPtr[%d]",
                        srcOffset, tempAlgParams.buffInfo.inputPtr);
                ...
                HCCL_DEBUG("[InsTempScatterMesh1D][RunMesh] tempAlgParam.buffInfo.hcclBuff.addr[%d], "
                ...
                HCCL_DEBUG("[InsTempScatterMesh1D][RunMesh] tempAlgParam.buffInfo.inBuffBaseOff[%d], "
        ...
        CHK_PRT_RET(channels.at(root_).empty(),
        ...
        curCount = curSliceSize / dataTypeSize;
        // 需要根据channel切分数据
        CHK_RET(CalcDataSplitByPortGroup(curCount, dataTypeSize, linkRecvs, elemCountOut_, sizeOut_, elemOffset_));
        // 循环每个channel接收数据
        for (u32 channelIdx = 0; channelIdx < channelsPerRank_; channelIdx++) {
