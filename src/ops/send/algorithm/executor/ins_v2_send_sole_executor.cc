...
template <typename InsAlgTemplate>
HcclResult InsV2SendSoleExecutor<InsAlgTemplate>::CalcRes(
...
template <typename InsAlgTemplate>
std::vector<CostModelParam> InsV2SendSoleExecutor<InsAlgTemplate>::CalcCostCoeff(
...
template <typename InsAlgTemplate>
HcclResult InsV2SendSoleExecutor<InsAlgTemplate>::CalcAlgHierarchyInfoV2(
    TopoInfoWithNetLayerDetails* topoInfo, AlgHierarchyInfoForAllLevel& algHierarchyInfo, const AlgAttrs& algAttrs)
{
    ...
    CHK_PRT_RET(
        (topoInfo->userRankSize == 0),
        HCCL_ERROR("[InsV2SendSoleExecutor][CalcAlgHierarchyInfoV2] Rank [%u], rankSize is 0.", myRank_),
        HcclResult::HCCL_E_PARA);
    ...
    HCCL_INFO("[InsV2SendSoleExecutor][CalcAlgHierarchyInfoV2][%u] Success.", myRank_);
    return HCCL_SUCCESS;
}

template <typename InsAlgTemplate>
HcclResult
InsV2SendSoleExecutor<InsAlgTemplate>::Orchestrate(const OpParam& param, const AlgResourceCtxSerializable& resCtx)
{
...
REGISTER_EXECUTOR_IMPL_NO_TOPOMATCH(HcclCMDType::HCCL_CMD_SEND, DpuSendSoleMesh, InsV2SendSoleExecutor, InsTempSendDpu);
REGISTER_ALG_ATTRS(
    DpuSendSoleMesh, topo.isSupportLevel0PcieMix = true; topo.isHostDpuOnly = true;
