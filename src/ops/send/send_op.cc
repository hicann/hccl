...
HcclResult
HcclSendNext(void* sendBuf, uint64_t count, HcclDataType dataType, uint32_t destRank, HcclComm comm, aclrtStream stream)
{
    ...
    CHK_RET_AND_PRINT_IDE(
        SendExec(sendBuf, count, dataType, destRank, comm, stream, rankSize, OpMode::OPBASE, tag), tag.c_str());
    ...
    const char* tag, void** streams, size_t streamCount, void* scratchMemAddr, uint64_t scratchMemSize)
{
    ...
    // 参数获取和校验
    CHK_PRT_RET(
        count == 0, HCCL_WARNING("[HcclSendGraphMode] input count is 0, return send success"),
    ...
    // 执行Send
    CHK_RET_AND_PRINT_IDE(
        SendExec(sendBuf, count, dataType, destRank, comm, stream, rankSize, OpMode::OFFLOAD, opTag, resPack),
        opTag.c_str());
    ...
    const uint32_t destRank, u32& rankSize, u32& userRank, std::string& opTag)
{
    ...
    CHK_PRT_RET(
        userRank == destRank, HCCL_ERROR("[HcclSend] destRank cannot be equal to self."),
        HcclResult::HCCL_E_NOT_SUPPORT);
    if (opTag.empty()) {
        char commName[COMM_INDENTIFIER_MAX_LENGTH];
        CHK_RET(HcclGetCommName(comm, commName));
        opTag = "SendRecv_" + string(commName) + "_" + std::to_string(userRank) + "_" + std::to_string(destRank);
    }
    CHK_RET(HcclCheckTag(opTag.c_str()));
    CHK_RET_AND_PRINT_IDE(HcomCheckUserRank(rankSize, userRank), opTag.c_str());
    CHK_RET_AND_PRINT_IDE(HcomCheckUserRank(rankSize, destRank), opTag.c_str());
    return HcclResult::HCCL_SUCCESS;
}

HcclResult GenerateSendOpParam(
    OpParam& param, void* sendBuf, uint64_t count, HcclDataType dataType, uint32_t destRank, const HcclComm comm,
...
HcclResult SendExec(
    void* sendBuf, uint64_t count, HcclDataType dataType, uint32_t destRank, const HcclComm comm,
    ...
    const ResPackGraphMode& resPack)
{
    ...
    if (rankSize == 1) {
        HCCL_WARNING(
            "[SendExec][%s][%s] ranksize == 1, enter SingleRankProc", tagTemp.c_str(),
...
HcclResult SendEntryLog(
    void* sendBuf, uint64_t count, HcclDataType dataType, uint32_t destRank, aclrtStream stream, const std::string& tag,
    const std::string& opName, bool forceLog)
{
    if (forceLog || GetExternalInputHcclEnableEntryLog()) {
        ...
        s32 ret = snprintf_s(
            ...
            "tag[%s], sendBuf[%p], count[%llu], dataType[%s], destRank[%u], streamId[%d], deviceId[%d]", tag.c_str(),
            sendBuf, count, GetDataTypeEnumStr(dataType).c_str(), destRank, streamId, deviceId);
