    ...
    ThreadHandle* threads)
{
    ...
    for (auto i = 0; i < threadNum; i++) {
        HcclResult ret
            = simComm->independentOpThreadMgr_->HcclThreadAcquire(engine, 1, config[i].notifyNumPerThread, &threads[i]);
        if (ret != HCCL_SUCCESS) {
            return ret;
        }
    }
...
bool HcommIsSupportHcclThreadAcquireWithConfig() { return true; }
...
bool HcommIsSupportHcommThreadResAcquireTimeOut() { return false; }

bool HcommIsSupportHcommSetNotifyWaitTimeOut() { return false; }

bool HcommIsSupportHcommThreadNotifyWaitOnThreadWithDefaultTimeout() { return false; }

bool HcommIsSupportHcommChannelNotifyWaitOnThreadWithDefaultTimeout() { return false; }

bool HcommIsSupportHcommChannelNotifyWaitWithDefaultTimeout() { return false; }
